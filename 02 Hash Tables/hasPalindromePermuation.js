...
function hasPalindromePermutation(theString) {
  ...
  const oddTallyLetters = new Set();

  for (let i = 0 ; i < theString.length; i++) {
    const letter = theString[i];
    if (oddTallyLetters.has(letter)) {
      oddTallyLetters.delete(letter);
    }
    else {
      oddTallyLetters.add(letter);
    }
  }

  return oddTallyLetters.size <= 1;
}


const theString = "ototo";
